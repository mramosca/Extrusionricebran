...
str(metadata)
colnames(metadata)[4] = "Property"

dataset <- merge(metadata, dataset, by.x = "SampleID", by.y = 0)
treatments<-as.vector(unique(dataset$Property))
datasetn<-dataset
datasetn[datasetn==0]<-NA

#i = 1

summary(metadata$Property)

# Filtrar metadata y dataset para eliminar los "Blank"
metadata <- subset(metadata, Property != "CONTROL")
dataset <- subset(dataset, Property != "CONTROL")
datasetn <- subset(datasetn, Property != "CONTROL")
...
write.csv(subset(strong_results_taxa, trt=="SOLUBLE"), "output/moving-pictures-strong-results-taxa-SOLUBLE.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="INSOLUBLE"), "output/moving-pictures-strong-results-taxa-INSOLUBLE.csv", row.names = F)
